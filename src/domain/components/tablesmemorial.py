...
from src.schemas.tableschemas import (
    estilo_assinatura,
    estilo_tabela_parametros,
    estilotabela,
    estilotabelaloc,
    styles,
)
...
class TablesBuilder:
    ...
    def tabela_localizacao(self):
        loc_instalacao = [
            ["COORDENADAS - coordenadas decimais - WGS 84 "],
            [
            ...
            [
                "",
                f"{self.retorno.latitude_obra}",
                f"{self.retorno.longitude_obra}",
            ],
        ]
    ...
    def tabelapainel(self):
        ...
        modulo_caracteristicas = [
            [
                "Potência nominal máx. (Pmax) ",
                f"({self.retorno.texto_potencia_placa} )Wp",
            ],
            [
                "Tensão operacional opt. (Vmp) ",
                f"({self.retorno.texto_tensao_individual_paineis} )V",
            ],
            [
                "Corrente operacional opt. (Imp)",
                f"({self.retorno.corrente_mp}) A",
            ],
            [
                "Tensão circuito aberto (Voc) ",
                f"({self.retorno.tensao_circuito_aberto} )V",
            ],
            [
                "Corrente curto-circuito (Isc)",
                f"({self.retorno.corrente_cc}) A",
            ],
        ]
    ...
    @staticmethod
    def tabela_parametros_tensao_inversor():
        # tabela parametrização inversor
        parametros_tensao_inversor = [
            [
                "Faixa de tensão no ponto de conexão [V]",
    ...
    @staticmethod
    def tabela_parametros_frequencia_inversor():
        parametros_frequencia_inversor = [
            [
                "Faixa de freqüência no ponto de conexão (Hz)",
                "Tempo de desconexão [s]",
            ],
            ["f ≤ 57,5", "0,2"],
        ...
        tabela_parametros_frequencia_inversor = Table(
            parametros_frequencia_inversor
        )
        tabela_parametros_frequencia_inversor.setStyle(
            estilo_tabela_parametros
        )
        return tabela_parametros_frequencia_inversor

    def tabela_parametros_fp_inversor(self):
        parametros_fp_inversor = [
            [
                "Potência Nominal (W) - Pn",
                "Faixa de fator de potência",
                "Fator de potência \nconfiguração em fábrica",
            ],
            [
                f"{self.retorno.potencia_inversores}",
    ...
    def tabela_queda_tensao(self):
        ...
        queda_tensao = [
            ["ρ  - resistividade do cobre", "0,0173"],
            [
                Paragraph(
                    "L<sub>c</sub> - comprimento do condutor",
                    styles["CorpoTexto"],
                ),
                "10 m",
            ],
            [
                Paragraph(
                    "I<sub>c</sub> - corrente do condutor",
                    styles["CorpoTexto"],
                ),
                f"{self.retorno.corrente_saida_por_inversor} A",
            ],
            ["Cosφ - fator de potencia", "1"],
            [
                Paragraph(
                    "S<sub>c</sub> - Seção reta do condutor",
                    styles["CorpoTexto"],
                ),
                f"{self.retorno.texto_cabos}mm²",
            ],
            [
                Paragraph("V <sub>f</sub> - tensão ", styles["CorpoTexto"]),
                f"{self.retorno.inversor_tensao} v",
            ],
        ]
    ...
    @staticmethod
    def tabela_assinatura(cliente_nome, cliente_cpf, data):
        # assinatura do cliente.
        assinatura = [
            [""],
            [
                f"                  {cliente_nome}, CPF:{cliente_cpf}                    "
            ],
            [f"{data}"],
        ]
